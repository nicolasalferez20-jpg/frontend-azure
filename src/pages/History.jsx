...
export default function History() {
  ...
  const historialFiltrado = (data || []).filter((item) => {
    ...
    // Si no hay texto escrito, mostrar todo
    if (!texto) return true;
    return (
      item.idHu.toString().includes(texto) ||
    ...
    <div className="flex min-h-screen bg-[#f8fafc] font-sans antialiased text-slate-800 w-full">
      ...
      <main className="flex-1 p-10 overflow-y-auto">
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-[0_2px_8px_rgba(0,0,0,0.02)] mb-6 flex flex-col md:flex-row items-end md:items-center gap-4 w-full">
          ...
          {/* Más filtros */}
          <button
            className="p-2 border border-slate-200 bg-white hover:bg-slate-50 rounded-lg text-slate-500 transition-colors cursor-pointer"
            title="Más filtros"
          >
        ...
        <div className="bg-white border border-slate-200 rounded-xl shadow-[0_4px_16px_rgba(0,0,0,0.02)] overflow-hidden w-full">
          ...
          {error && (
            <div className="flex flex-col items-center justify-center py-16 gap-2 text-red-500">
              ...
              <p className="text-sm font-semibold text-slate-800">
                Error al enlazar con el servidor externo
              </p>
            </div>
          )}
            ...
            <div className="overflow-x-auto w-full">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-100 text-sm font-medium text-slate-700">
                  {historialFiltrado.map((pdf) => {
                    const isError =
                      pdf.estado?.toLowerCase() === "error" ||
                      (!pdf.urlArchivo && !pdf.url_archivo);
                    const isProcessing =
                      pdf.estado?.toLowerCase() === "en proceso";
                          ...
                          <div className="flex items-center justify-center gap-3">
                            {isError ? (
                              <>
                                ...
                                <RefreshCw
                                  size={16}
                                  className="text-slate-400 hover:text-[#0078d4] cursor-pointer"
          ...
          <div className="p-4 bg-slate-50/50 border-t border-slate-200 flex flex-col sm:flex-row justify-between items-center gap-4 px-6">
            <span className="text-xs text-slate-500 font-medium">
              Mostrando {historialFiltrado.length} de {data?.length || 0}{" "}
              reportes
            </span>
            ...
            <div className="inline-flex shadow-xs rounded-lg border border-slate-200 bg-white text-sm font-semibold overflow-hidden">
              <button className="px-3 py-1.5 text-slate-400 hover:bg-slate-50 border-r border-slate-200 transition-colors cursor-not-allowed">
                &lt;
              </button>
              <button className="px-3.5 py-1.5 text-slate-600 hover:bg-slate-50 border-r border-slate-200 transition-colors">
                1
              </button>
              <button className="px-3.5 py-1.5 text-slate-600 hover:bg-slate-50 border-r border-slate-200 transition-colors">
                2
